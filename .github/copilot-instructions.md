...
All paths are dynamically generated from `package.json` → `name` field to support easy project renaming:

- **Central Configuration**: `node_scripts/get-addon-name.js` provides `getAddonName()` and `getAddonPaths()` utilities
- **Usage in Scripts**: All Node.js scripts import from `get-addon-name.js` instead of hardcoding paths
- **Build Integration**: Rollup plugins receive `addonName` from `packageInfo` parameter
- **VScripts Config Module**: Auto-generated `_config.ts` provides project constants (ADDON_NAME, DEBUG_TAG_*) for business code
- **Key Benefit**: Change `package.json` → `name` to rename entire addon - no path updates needed

```javascript
// Example usage in node scripts
const { getAddonPaths } = require('./get-addon-name');
const paths = getAddonPaths();
// Returns: contentRoot, gameRoot, contentScripts, gameScripts, etc.
```

```typescript
// Example usage in VScripts business code
import { ADDON_NAME, GetDebugTag } from "./_config";
const tag = GetDebugTag(); // Returns "solid_template_debug" or "solid_template_client_debug"
```

**Path Structure:**
```
content/{addon_name}/panorama/          # Compiled Panorama UI output
content/{addon_name}/scripts/vscripts/  # TypeScript VScripts source
content/{addon_name}/scripts/vscripts/_config.ts  # Auto-generated project constants
content/{addon_name}/declarations/      # Type definitions for VScripts
game/{addon_name}/scripts/vscripts/     # Compiled Lua output
game/{addon_name}/scripts/npc/          # NPC/KV data files
```
...
**Build Commands:**
```bash
npm run generate:config  # Generate tsconfig.json files (auto-run by build commands)
...
- Auto-generates `tsconfig.json` and `_config.ts` before starting TSTL compiler
...
- Converts paths from `content/{addon_name}/scripts/vscripts/*.ts` → `game/{addon_name}/scripts/vscripts/*.lua`
- All paths dynamically read from `getAddonPaths()` utility

**TypeScript Configuration:**
- `tsconfig.json` files are auto-generated by `node_scripts/generate-tsconfig.js`
- `_config.ts` is auto-generated in VScripts directory with project constants
- Automatically run before any build/watch command
- Configuration includes proper paths based on current addon name
- Manual edits to `tsconfig.json` or `_config.ts` will be overwritten on next build
- To customize: modify `generate-tsconfig.js` instead

**VScripts Config Pattern:**
Business code should use `_config.ts` instead of hardcoding project names:
```typescript
// ❌ Bad - hardcoded
const tag = IsServer() ? "solid_template_debug" : "solid_template_client_debug";
