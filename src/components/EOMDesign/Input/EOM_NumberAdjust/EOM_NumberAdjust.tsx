import { ParentComponent, createRenderEffect, createSignal, mergeProps, splitProps } from "solid-js";
import { ADDON_NAME, EOMProps, EOM_Attribute } from "../../EOMDesign";
import { EOM_BaseButton } from "../EOM_Button/EOM_Button";
import "./EOM_NumberAdjust.less";

interface EOM_NumberAdjustAttribute extends EOM_Attribute {
...
export const EOM_NumberAdjust: ParentComponent<EOM_NumberAdjustAttribute & NumberEntryAttributes> = (props) => {
	const merged = mergeProps({
		min: 0,
		max: 1000000,
		type: ADDON_NAME
	}, props);
	const [local, others] = splitProps(merged, ["children", "type", "effectValue", "value", "min", "max", "onChange", "onvaluechanged", "changeFilter"]);
	...
	return <Panel
		{...EOMProps(others, {
			class: "EOM_NumberAdjust " + local.type,
		})}
	>
		...
		<Panel className="ChangeValueButtons">
			<EOM_BaseButton enabled={value() < local.max} onactivate={self => {
				if (value() == undefined) {
					setValue(local.max);
			...
			<EOM_BaseButton enabled={value() > local.min} marginTop="3px" onactivate={self => {
				if (value() == undefined) {
					setValue(local.min);
