...
import { For, Match, ParentComponent, Show, Switch, createEffect, createMemo, createSignal, mergeProps, onCleanup, onMount, splitProps } from "solid-js";
import { createStore } from "solid-js/store";
import { useNetTable } from "solid-panorama-all-in-jsx/events.macro";
import { GetLocalization } from "../../../../utils/localization";
import { createServiceNetTableData } from "../../../../utils/solid_utils";
import { CustomItemImage } from "../../../AbilityImage/AbilityImage";
...
import { EOM_Attribute } from "../../EOMDesign";
...
interface EOM_DebugToolAttribute extends EOM_Attribute {
	/** 面板的方向 */
...
const player_key_values = createServiceNetTableData("player_key_values", {});
			...
			<Panel id="EOM_DebugToolControlPanel" className={classNames("ControlPanel", {
				...
				<Panel className={"ControlPanelContainer TabShow" + tabIndex()}>
					<Panel className="ControlPanelTitle">
						...
						{/* <CLabel className="CategoryHeader" text={`工具(FPS:${fps()})`} /> */}
						<Switch fallback={<CLabel className="CategoryHeader" text={`工具`} />}>
						...
						<EOM_IconButton className="CategoryHeaderIcon" tooltip="切换布局" verticalAlign="center" icon={<EOM_Icon type="Popout" size="24" />} >
							<EOM_DropDown id="ToggleSize" type="" onChange={(index, item) => { setDirection(item.id); SaveConfig("direction", item.id); }}>
								<CLabel text={"左侧"} id="left" />
								<CLabel text={"上方"} id="top" />
								<CLabel text={"右侧"} id="right" />
							</EOM_DropDown>
						</EOM_IconButton>
...
function EOM_DebugTool_Setting() {
	...
	return (
		<SelectionContainer
			...
			hasFilter={false}
		>
			<EOM_Panel id="EOM_DebugTool_Setting" className="EOM_DebugTool_Setting" flowChildren="down" width="100%" height="100%">{/* 面板热键 */}
				{/* 面板热键 */}
					...
					<For each={Object.keys(playerConfig())}>
						{(eventName, index) => {
							if (eventName.indexOf("hotkey_") != -1 && defaultSettingList.indexOf(eventName.replace("hotkey_", "")) == -1) {
								return (
									<EOM_Panel width="100%" height="36px" flowChildren="right" className={classNames("CanRemoveKeyBind", { deleteMode: deleteMode })}>
										<EOM_IconButton verticalAlign="center" icon={<EOM_Image src="s2r://panorama/images/control_icons/x_close_png.vtex" />} onactivate={self => {
				...
				<EOM_Button verticalAlign="bottom" type="Outline" color="Blue" text="清空设置" onactivate={() => {
					{
	...
	return (
		<Panel className={classNames("Category", "TabIndex" + (props.tabIndex ?? ""), { SingleCol: col <= 1 })}>
			<Panel className="CategoryHeader">
				<CLabel className="CategoryHeaderLabel" text={props.title} />
			...
			<Panel className="CategoryButtonContainer">
				{!layout && [...Array(Math.ceil((props.children?.length ?? 0) / col))].map((_, rowIndex) => {
					return (
						<Panel className="Row">
	...
	return (
		<Panel className="DemoSlider">
			<EOM_Panel flowChildren="right">
				<CLabel className="Title" text={props.text} />
				<CLabel className="Value" text={value().toFixed(0)} />
			</EOM_Panel>
...
/** 选择容器 */
interface SelectContainerAttribute extends EOM_Attribute {
	/** 事件名 */
	eventName: string,
	/** 标题 */
	title: string,
...
export const SelectionContainer: ParentComponent<SelectContainerAttribute> = (props) => {
	...
	return (
		<EOM_Panel id={local.eventName} className={classNames("SelectionContainer", { LockWindow: lock() })} hittest={true} width={size().width} height={size().height}>
			<Panel id="SelectionPicker" >
				<Panel id="SelectionPickerHeader" onactivate={() => { }} onmouseover={self => dragStart(self)} onmouseout={self => dragable = false} >
					<CLabel id="SelectionTitle" text={local.title} />
						...
						<Panel id="SelectionSearch" className="SearchBox" >
							<Show when={hasToggleList()}>
								<EOM_DropDown type="" placeholder="筛选" onChange={(index, item) => {
									...
									<CLabel id="EOM_DropDown_Clear" text="X 清除筛选" />
									<For each={Object.keys(local.toggleList ?? {})}>
										{(key, index) => (
											// @ts-ignore
											<CLabel text={local.toggleList?.[key] ?? ""} />
										)}
									</For>
								</EOM_DropDown>
							</Show>
...
export const EOM_DebugTool_TextPicker: ParentComponent<{
	...
	return (
		...
		// height="360px"
		>
			<EOM_Panel className="EOM_DebugTool_TextPicker" flowChildren="right-wrap" width="100%" height="100%" scroll="y" >
				...
				{local.itemNames?.map((abilityname, index) => {
					...
					return (
						<EOM_BaseButton className="EOM_DebugTool_AbilityPickerItem" width="64px" flowChildren="down" onactivate={self => FireEvent(local.eventName, abilityname)} onmouseover={p => CustomUIConfig.ShowAbilityTooltip<AbilityTooltip>(p, { abilityname: abilityname })} onmouseout={p => CustomUIConfig.HideAbilityTooltip(p)}>
							...
							<CLabel className="EOM_DebugTool_AbilityPickerItemName" text={rawMode() ? abilityname : "#DOTA_Tooltip_ability_" + abilityname} />
						</EOM_BaseButton>
					);
				})}
			</EOM_Panel>
...
export const EOM_DebugTool_ItemPicker: ParentComponent<{
	...
	return (
		<SelectionContainer
			...
			canScroll={false}
		>
			<EOM_Panel width={"100%"} height="100%" margin="-6px" backgroundColor="#00000066" >
				<EOM_Panel className="EOM_DebugTool_AbilityPicker" marginTop="10px" paddingTop="30px" flowChildren="right-wrap" width="100%" scroll="y" >
					<For each={local.itemNames}>
						{(abilityname, index) => <EOM_BaseButton visible={visiable(abilityname)} className="EOM_DebugTool_AbilityPickerItem" width="64px" flowChildren="down" onactivate={self => FireEvent(local.eventName, abilityname)} onmouseover={p => CustomUIConfig.ShowAbilityTooltip<AbilityShopItemTooltip>(p, { abilityname: abilityname, guidename: "", entityindex: -1 as EntityIndex })} onmouseout={p => CustomUIConfig.HideAbilityTooltip(p)}>
							<CustomItemImage itemname={abilityname} showtooltip={false} />
							<CLabel className="EOM_DebugTool_AbilityPickerItemName" text={rawMode() ? abilityname : "#DOTA_Tooltip_ability_" + abilityname} />
						</EOM_BaseButton>}
					</For>
				</EOM_Panel>
...
export const EOM_UnitInfo: ParentComponent = (props) => {
	...
	return (
		<SelectionContainer
			...
			hasFilter={false}
		>
			<EOM_Panel width="100%" height="100%" ref={panel}>
						...
						{(itemIndex, index) => {
							if (itemIndex != -1) {
								...
								return <Panel class="DevouredItemActionPanel">
									<CustomItemImage id={"Drag" + abilityName} itemname={abilityName} showtooltip={false} onmouseover={(p) => CustomUIConfig.ShowAbilityTooltip<AbilityEntityTooltip>(p, { abilityentityindex: abilityentityindex, entityindex: unitInfo.entIndex })} onmouseout={(p) => CustomUIConfig.HideAbilityTooltip(p)} />
