...
export const EOM_KeyBinder: ParentComponent<EOM_KeyBinderProps> = (props) => {
	const merged = mergeProps({ type: KeyBinderType.Normal, text: "", initKey: "", initDropIndex: 0 }, props);
	...
	return (
		<EOM_Panel id="EOM_KeyBinder" ref={panel} {...EOMProps(others, {
			className: classNames("SettingsKeyBinder", "BindingRow", {
				HeroAbilityBindAbilityButton: local.type == KeyBinderType.Ability,
				ItemBindButton: local.type == KeyBinderType.Item,
			})
			// @ts-expect-error
		})} onactivate={self => OnActivate(self)} onfocus={self => OnFocus(self)} onblur={self => OnBlur(self)} oncancel={self => OnCancel(self)} onload={self => OnLoad(self)} tooltip={local.tooltip}>
			<Show when={typeof local.text == "string"}>
				<CLabel id="title" text={local.text as string} className="BindingRowLabel" html={true} />
			</Show>
			<Panel id="LabelFXContainer">
				<Panel id="BindingLabelContainer" >
					<CLabel id="mod" text="" className="BindingRowButton" />
					<CLabel id="dash" text="-" className="BindingRowButton" />
					<CLabel id="value" text={GetLocalization("#" + keyName()) == "#" + keyName() ? keyName() : ("#" + keyName())} className="BindingRowButton" />
				</Panel>
